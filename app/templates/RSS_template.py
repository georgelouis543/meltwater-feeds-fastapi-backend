...
def return_rss_output(
        feed_metadata: HtmlRssFeedRequest,
        items: list,
) -> str:
    ...
    rss_output += f'<rss version="2.0" xmlns:media="http://search.yahoo.com/mrss/">\n'
    ...
    for item in items:
        ...
        rss_output += item["description"]
        rss_output += f'</description>\n'
        rss_output += f'<guid ispermalink="false">{item["_id"]}</guid>\n'

        if feed_metadata["use_index_date"] is True or not item["published_date"]:
            rss_output += f'<pubDate>{item["indexed_date"]}</pubDate>\n'
        else:
