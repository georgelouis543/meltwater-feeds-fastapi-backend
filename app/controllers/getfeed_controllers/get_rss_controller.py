...
from app.controllers.getfeed_controllers.get_merged_feed_items import get_merged_items_handler
...
async def get_rss_feed(
    ...
    try:
        ...
        if feed_metadata["feed_type"] == "merged_feed":
            merged_items = await get_merged_items_handler(
                feed_metadata["feed_ids"],
                documents_collection
            )
            return return_rss_output(
                MergeFeed(**feed_metadata),
                merged_items
            )
